  ...
  try {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        // This is not the right way to get the user id, but for now we will just use the email as the user id
        userId = data.email;
      } else {
    ...
    // ---------------------------
    await setDoc(doc(db, 'users', userId), { user_id: userId, email: data.email, role: 'tourist' }, { merge: true });

    // ---------------------------
    // Step 3: Check existing tourist profile
    // ---------------------------
    const profileDoc = await getDoc(doc(db, 'tourist_profiles', userId));
    ...
    // ---------------------------
    await setDoc(doc(db, 'user_preferences', userId), { user_id: userId, language: 'en', notifications_enabled: true, location_sharing: true }, { merge: true });
...
const logAuditAction = async (userId: string, actionType: string, details: any) => {
  try {
    await addDoc(collection(db, 'audit_logs'), { action_by: userId, action_type: actionType, details, timestamp: new Date() });
  } catch (error) {
